...
const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        sendStickerPackMessage: async (jid, options = {}) => {
    const userJid = authState.creds.me.id;
    const { name, thumbnailUrl, stickerPackOrigin, publisher, urls = [] } = options;
    const uploadedStickers = await Promise.all(urls.map(async (stickerInfo) => {
        const { url, emojis = [], accessibilityLabel = '' } = stickerInfo;
        const isAnimated = url.endsWith('.gif') || url.endsWith('.webp');
        const isLottie = url.endsWith('.json');
        const mimetype = isLottie ? 'application/json' : (isAnimated ? 'image/webp' : 'image/webp');
        const buffer = await axios.get(url, { responseType: 'arraybuffer' }).then(r => r.data);
        const uploadResult = await waUploadToServer(buffer, { mimetype });
        return {
    ...
    let thumbnailDirectPath, thumbnailSha256, thumbnailEncSha256;
    if (thumbnailUrl) {
        const thumbBuffer = await axios.get(thumbnailUrl, { responseType: 'arraybuffer' }).then(r => r.data);
        const thumbUpload = await waUploadToServer(thumbBuffer, { mimetype: 'image/jpeg' });
        thumbnailDirectPath = thumbUpload.directPath;
        thumbnailSha256 = thumbUpload.fileSha256B64;
        thumbnailEncSha256 = thumbUpload.fileEncSha256B64;
    }
    const packMessage = await Utils_1.generateWAMessageFromContent(jid, {
        stickerPackMessage: {
            name,
            publisher,
            ...
            thumbnailEncSha256: thumbnailEncSha256 ? Buffer.from(thumbnailEncSha256, 'base64') : undefined,
            stickerPackOrigin
        }
    }, { userJid });
    await relayMessage(jid, packMessage.message, { messageId: packMessage.key.id });
    return packMessage;
},
