...
const makeMessagesSocket = (config) => {
    ...
    if (typeof content === 'object' &&
    ...
    else if (typeof content === 'object' && 'pin' in content && content.pin) {
        const { pin } = content
        const { type, time, key } = pin

        if (!key || !key.id) {
            throw new Error('Invalid message key provided for pinning.')
        }

        if (WABinary_1.isJidStatus(jid)) {
            throw new Error('Cannot pin messages in status.')
        }

        const pinMessageProto = {
            protocolMessage: {
                type: WABinary_1.isJidNewsletter(jid) ? 16 : 15,
                key: {
        ...
        const fullMsg = await Utils_1.generateWAMessage(jid, pinMessageProto, {
            logger,
            userJid,
        ...
        const additionalAttributes = {
            edit: WABinary_1.isJidNewsletter(jid) ? '3' : '1'
        }
        ...
        const isPin = 'pin' in content && content.pin && content.pin.type === 1
        const isUnpin = 'pin' in content && content.pin && content.pin.type === 0
        const additionalAttributes = {}
